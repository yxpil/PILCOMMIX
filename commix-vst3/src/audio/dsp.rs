...
// ============ 4096 点正弦查表(DX7 硬件模拟:无插值,直接截断 = 硬件量化) ============
pub const SINE_LUT_SIZE: usize = 4096;
static SINE_LUT: OnceLock<Vec<f32>> = OnceLock::new();
/// 相位以"圈"为单位(0-1);查表取整截断,模拟硬件查表量化
#[inline]
pub fn sine_lut(turns: f64) -> f32 {
    let t = SINE_LUT.get_or_init(|| {
        (0..SINE_LUT_SIZE)
            .map(|i| (2.0 * std::f32::consts::PI * i as f32 / SINE_LUT_SIZE as f32).sin())
            .collect()
    });
    let idx = ((turns.fract() * SINE_LUT_SIZE as f64) as usize) & (SINE_LUT_SIZE - 1);
    t[idx]
}
...
#[derive(Clone, Copy, Debug)]
pub struct Biquad {
    pub kind: u8,        // 0=lowpass 1=highpass 2=peaking(EQ)
