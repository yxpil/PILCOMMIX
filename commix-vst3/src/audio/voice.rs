...
use super::dsp::{sr, Adsr, Biquad, ShLfo};
...
struct DxVoice {
    ...
    use_lut: bool,      // 4096 正弦查表
    quant: bool,        // 16bit 定点截断
    dac: bool,          // DAC 输出量化
...
impl DxVoice {
    fn new(p: &EngineParams) -> Self {
        ...
        Self {
            ...
            use_lut: p.dx_lut, quant: p.dx_quant, dac: p.dx_dac,
...
impl Voice {
    ...
    fn build_voice(&mut self, p: &EngineParams) {
        ...
        match self.wave_type.as_str() {
            ...
            "wt" => {
                let mut flt = Biquad::new(if p.filter_kind == "highpass" { 1 } else { 0 }, cutoff_eff(p, self.midi, self.vel), p.resonance_q);
                flt.reset();
                self.filter = Some(flt);
    ...
    pub fn apply_jitter(&mut self, amount: f32) {
        let r = |lo: f32, hi: f32| lo + (hi - lo) * super::dsp::rand01();
    ...
    fn render_dx(&mut self, dt: f32) -> f32 {
        ...
        for (i, op) in dx.ops.iter_mut().enumerate() {
            ...
            op.phase += if dx.quant { super::dsp::q16(inc as f32) as f64 } else { inc };
            eg_vals[i] = op.eg.next(dt);
        }
        let sinv = |ph: f64| {
            let v = if dx.use_lut {
                super::dsp::sine_lut(ph.fract())
            } else {
                (ph.fract() * std::f32::consts::TAU as f64).sin() as f32
            };
            if dx.quant { super::dsp::q16(v) } else { v }
        };
