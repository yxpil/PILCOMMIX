...
// ============ 合唱 Chorus(经典调制效果) ============
// 短延迟线 + 正弦 LFO 调制延迟时间(0.5-4ms 扫动),输出 = 干 + 湿混合。
// 效果:声音"加厚/群感",立体声宽度感;深度调大可到"合唱团"效果。
// 位置:主效果链延迟之后、限制器之前(与混响/延迟同级)。
pub struct Chorus {
    buf: Vec<f32>,
    idx: usize,
    phase: f32,
    pub rate: f32,      // LFO 速率 Hz(0.1-8,默认 1.5)
    pub depth_ms: f32,  // 延迟调制深度 ms(0-6,默认 2)
    pub mix: f32,       // 湿声混合 0-1(默认 0 = 关闭直通)
}
impl Chorus {
    pub fn new() -> Self {
        Self {
            buf: vec![0.0; (2.0 * sr()) as usize],  // 2s 环形缓冲(足够大)
            idx: 0,
            phase: 0.0,
            rate: 1.5,
            depth_ms: 2.0,
    ...
    pub fn process(&mut self, x: f32, dt: f32) -> f32 {
        ...
        self.buf[self.idx] = x;
        // LFO:0-1 三角正弦扫动 → 延迟 0.5ms 基础 + depth
        self.phase += dt * self.rate * std::f32::consts::TAU;
        let lfo = 0.5 - 0.5 * self.phase.sin();   // 0..1
        let delay_s = (0.0005 + lfo * self.depth_ms / 1000.0).clamp(0.0002, 0.02);
        let delay = (delay_s * sr()) as usize;
        let read = (self.idx + len - delay.min(len - 1)) % len;
        let delayed = self.buf[read];
        x + (delayed - x) * self.mix   // dry/wet 交叉混合
    }
}
