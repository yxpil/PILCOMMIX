...
use super::waves::smooth_interp_anchors;
...
pub struct EngineParams {
    ...
    // 粒子合成(波形表粒子流:尺寸/密度/散布/随机)
    pub grain_size_ms: f32,   // 粒子时长 ms(10-500,默认 80)
    pub grain_density: f32,   // 粒子密度 个/秒(5-200,默认 40)
    pub grain_spread: f32,    // 粒子音高散布 音分(0-200,默认 30)
    pub grain_random: f32,    // 粒子相位/位置随机 0-1(默认 0.3)
    // 粒子参数包络(扩展):时长/密度随时间从起始值滑动到终点值
    pub grain_size_end: f32,  // 粒子时长终点 ms(与 size 相同 = 直线;不同 = 曲线)
    pub grain_density_end: f32, // 粒子密度终点 个/秒
    pub grain_env_ms: f32,    // 包络滑动时间 ms(0-3000,默认 800)
    pub grain_env_exp: f32,   // 包络形状指数(0=线性,>0 前快后慢,<0 前慢后快)
    // DX7 硬件模拟(轻量路线:PM 相位调制 + 4096 正弦表 + 定点截断 + 仅 OP6 反馈)
    pub dx_pm: bool,       // 切换为 PM 相位调制(默认 false=标准 FM)
    pub dx_lut_size: u32,   // 正弦查表尺寸:0=关(浮点 sin),1024/2048/4096/8192
    pub dx_quant_bits: u8,  // 定点截断位数:0=关,2/4/8/12/14/16
...
impl Default for EngineParams {
    fn default() -> Self {
        Self {
            ...
            grain_size_ms: 80.0, grain_density: 40.0, grain_spread: 30.0, grain_random: 0.3,
            grain_size_end: 80.0, grain_density_end: 40.0, grain_env_ms: 800.0, grain_env_exp: 0.0,
            dx_pm: false, dx_lut_size: 4096, dx_quant_bits: 0, dx_dac: false, dx_aa: false,
            dx_bits: 12,
...
impl EngineParams {
    ...
    /// set_params(整组灌入)与 SynthEngine::new 都调用,覆盖预设/Tauri 命令/VST 参数等全部入口。
    pub fn sanitize(&mut self) {
        ...
        self.grain_size_ms = self.grain_size_ms.max(10.0).min(500.0);
        self.grain_density = self.grain_density.max(5.0).min(200.0);
        self.grain_spread = self.grain_spread.max(0.0).min(200.0);
        self.grain_random = self.grain_random.max(0.0).min(1.0);
        self.grain_size_end = self.grain_size_end.max(10.0).min(500.0);
        self.grain_density_end = self.grain_density_end.max(5.0).min(200.0);
        self.grain_env_ms = self.grain_env_ms.max(0.0).min(3000.0);
        self.grain_env_exp = self.grain_env_exp.max(-3.0).min(3.0);
        self.dx_lut_size = match self.dx_lut_size { 1024 | 2048 | 8192 => self.dx_lut_size, _ => 4096 };
        self.dx_quant_bits = if self.dx_quant_bits == 0 { 0 } else { self.dx_quant_bits.clamp(2, 16) };
...
pub struct SynthEngine {
    ...
    pub custom_smooth_table: Vec<f32>,
...
impl SynthEngine {
    ...
    pub fn set_params(&mut self, mut p: EngineParams) {
        ...
        self.params = p;
        self.rebuild_tables();
        // 调制 LFO 参数刷新活跃音符(整组灌参数也实时生效)
        let (d, t, w) = (self.params.mod_lfo_depth, self.params.mod_lfo_target.clone(), self.params.mod_lfo_wave.clone());
        let target = match t.as_str() { "cutoff" => 1, "volume" => 2, "pan" => 3, _ => 0 };
        let wave_osc = w != "s&h";
        for v in self.voices.iter_mut() {
            v.mod_depth = d;
            v.mod_target = target;
            v.mod_wave_osc = wave_osc;
        }
    }
    ...
    pub fn set_param(&mut self, key: &str, v: f64) {
        {
            ...
            "filter_kind" => p.filter_kind = match v.round() as i32 {
                1 => "highpass".into(), 2 => "bandpass".into(), 3 => "notch".into(), _ => "lowpass".into(),
            },
            ...
            "dx_pm" => p.dx_pm = v > 0.5,
            "dx_lut" => p.dx_lut_size = if v > 0.5 { 4096 } else { 0 },   // 兼容旧前端
            "dx_lut_size" => p.dx_lut_size = match v.round() as u32 { 1024 | 2048 | 8192 => v.round() as u32, _ => 4096 },
            "dx_quant_bits" => {
                let b = v.round() as u8;
                p.dx_quant_bits = if b == 0 { 0 } else { b.clamp(2, 16) };
            },
            ...
            "dx_bits" => p.dx_bits = v.clamp(8.0, 16.0) as u8,   // DAC 输出量化(8/12/16)
            "grain_size_ms" => p.grain_size_ms = f.clamp(10.0, 500.0),
            "grain_density" => p.grain_density = f.clamp(5.0, 200.0),
            "grain_spread" => p.grain_spread = f.clamp(0.0, 200.0),
            "grain_random" => p.grain_random = f.clamp(0.0, 1.0),
            "grain_size_end" => p.grain_size_end = f.clamp(10.0, 500.0),
            "grain_density_end" => p.grain_density_end = f.clamp(5.0, 200.0),
            "grain_env_ms" => p.grain_env_ms = f.clamp(0.0, 3000.0),
            "grain_env_exp" => p.grain_env_exp = f.clamp(-3.0, 3.0),
            _ => {}
            }
        }
        // 算法/反馈变化:刷新活跃 DX 音符的算法拓扑(保留相位/包络状态;
        // 否则只对新音符生效,正在响的音符不变)
        if key == "dx_algorithm" {
            let alg_idx = match self.params.dx_algorithm {
                1 => 0, 2 => 1, 3 => 2, 4 => 3, 5 => 4, 6 => 5, 7 | 32 => 6, _ => 0,
            };
            let (mod_by, carriers) = super::voice::DX_ALGS[alg_idx.min(super::voice::DX_ALGS.len() - 1)];
            for v in self.voices.iter_mut() {
                if let Some(dx) = v.dx.as_mut() {
                    dx.mod_by = mod_by;
                    dx.carriers = carriers;
                }
            }
        }
        // 谐波数变化:重建波表并刷新活跃音符(实时生效)
        if key == "harmonics" { self.rebuild_tables(); }
        // 调制 LFO 参数变化:刷新活跃音符(否则只对新音符生效,正在响的音不变)
        if key.starts_with("mod_lfo") {
            let (d, t, w) = (self.params.mod_lfo_depth, self.params.mod_lfo_target.clone(), self.params.mod_lfo_wave.clone());
            let target = match t.as_str() { "cutoff" => 1, "volume" => 2, "pan" => 3, _ => 0 };
            let wave_osc = w != "s&h";
            for v in self.voices.iter_mut() {
    ...
    pub fn set_custom_anchors(&mut self, anchors: Vec<(f32, f32)>) {
        // 一次构建两张表(线性锚点表 + 平滑 Catmull-Rom 表),避免重复构建
        let n = 2048usize;
        self.custom_table = Vec::with_capacity(n);
        self.custom_smooth_table = Vec::with_capacity(n);
        for i in 0..n {
            let px = i as f32 / n as f32;
            self.custom_table.push(super::waves::interp_anchors(&anchors, px));
            self.custom_smooth_table.push(smooth_interp_anchors(&anchors, px));
        }
    ...
    fn rebuild_tables(&mut self) {
        ...
        let custom_src = if self.custom_table.is_empty() {
            super::waves::build_wavetable("saw", self.params.harmonics as usize)
        } else {
        ...
        for v in self.voices.iter_mut() {
            match v.wave_type.as_str() {
                ...
                "grain" => v.wt_bank = vec![custom_src.clone()],   // 粒子源实时更新
    ...
    pub fn note_on(&mut self, midi: u8, velocity: f32, t: f32) {
        ...
        let wt_bank = if p.wave_type == "wt" {
            self.wt_bank.clone()
        } else if p.wave_type == "grain" {
            // 粒子合成:粒子源 = 自定义波形优先(平滑插值,无折线感),否则锯齿
            vec![if self.custom_table.is_empty() {
                super::waves::build_wavetable("saw", p.harmonics as usize)
            } else {
                self.custom_smooth_table.clone()
            }]
...
mod mod_lfo_tests {
    use super::*;
    #[test]
    fn mod_depth_updates_active_voice() {
        // 按下音符后修改调制深度,正在响的音符必须实时变化(输出不同)
        let mut p = EngineParams::default();
        p.wave_type = "saw".into();
        p.cutoff_hz = 2000.0;
        p.mod_lfo_target = "cutoff".into();
        p.mod_lfo_depth = 0.0;
        let mut e = SynthEngine::new(p);
        e.note_on(60, 1.0, 0.0);
        // 深度 0:渲染 0.5s 取能量
        ...
        e.render_block(&mut l, &mut r, 22050, 0.0);
        let e0: f32 = l[10000..].iter().map(|x| x * x).sum();
        // 深度 0.8:实时 set_param(活跃音符应跟随)
        e.set_param("mod_lfo_depth", 0.8);
        let mut l2 = vec![0.0f32; 22050];
        let mut r2 = vec![0.0f32; 22050];
        e.render_block(&mut l2, &mut r2, 22050, 0.0);
        let e1: f32 = l2[10000..].iter().map(|x| x * x).sum();
        // 调制滤波会改变频谱/能量,两段应显著不同
        let diff = (e0 - e1).abs() / e0.max(1e-9);
        println!("mod test: e0={e0} e1={e1} diff={diff}");
        assert!(diff > 0.02, "调制深度应实时改变活跃音符输出, diff={diff}");
    }
}
...
mod lut_quant_tests {
    use super::*;
    fn render_peak(p: &EngineParams) -> f32 {
    ...
    #[test]
    fn lut_sizes_all_render() {
        for size in [0u32, 1024, 2048, 4096, 8192] {
            let mut p = EngineParams::default();
            p.wave_type = "dx7".into();
            p.dx_pm = true; p.dx_lut_size = size;
            let pk = render_peak(&p);
            assert!(pk > 0.05, "lut_size={size} 峰值过低 {pk}");
        }
    }
    #[test]
    fn lut_size_audibly_different() {
        // 相位寄存器量化:1024 与 8192 的输出应有明显差异(量化噪声)
        // 若只是查值截断(相位浮点累积),两者几乎相同——此测试保证硬件行为
        fn render(p: &EngineParams) -> Vec<f32> {
            let mut e = SynthEngine::new(p.clone());
        ...
        let mut p = EngineParams::default();
        p.wave_type = "dx7".into();
        p.dx_pm = true; p.dx_aa = true;
        p.dx_lut_size = 1024;
        let a = render(&p);
        p.dx_lut_size = 8192;
        let b = render(&p);
        let diff: f32 = a.iter().zip(b.iter()).map(|(x, y)| (x - y) * (x - y)).sum::<f32>() / a.len() as f32;
        let energy: f32 = a.iter().map(|x| x * x).sum::<f32>() / a.len() as f32;
        let ratio = (diff / energy.max(1e-6)).sqrt();
        assert!(ratio > 0.01, "1024 vs 8192 应可感知差异,实际误差比 {ratio:.4}");
    }
    #[test]
    fn quant_bits_change_output() {
        let mut p = EngineParams::default();
        p.wave_type = "dx7".into();
        p.dx_pm = true; p.dx_lut_size = 4096;
        p.dx_quant_bits = 0;
        let clean = render_peak(&p);
        p.dx_quant_bits = 2;
        let rough = render_peak(&p);
        assert!((clean - rough).abs() > 1e-4, "2bit 量化应有差异 clean={clean} rough={rough}");
    }
}

#[cfg(test)]
mod noise_chorus_tests {
    use super::*;
    #[test]
    fn noise_tone_renders() {
        let mut p = EngineParams::default();
        p.wave_type = "noise".into();
        p.filter_kind = "highpass".into();
        p.cutoff_hz = 800.0;
        let mut e = SynthEngine::new(p);
        e.note_on(60, 1.0, 0.0);
        let n = 11025usize;
        ...
        assert!(peak > 0.05, "噪声音色应出声,峰值 {peak}");
    }
}
...
mod grain_tests {
    use super::*;
    #[test]
    fn grain_tone_renders_and_density_matters() {
        fn render(density: f32) -> f32 {
            let mut p = EngineParams::default();
            p.wave_type = "grain".into();
            p.grain_size_ms = 80.0;
            p.grain_density = density;
            p.grain_spread = 30.0;
            let mut e = SynthEngine::new(p);
            e.note_on(60, 1.0, 0.0);
            let n = 22050usize;
            ...
            let mut r = vec![0.0f32; n];
            e.render_block(&mut l, &mut r, n, 0.0);
            let e2: f32 = l.iter().chain(r.iter()).map(|x| x * x).sum::<f32>() / (n * 2) as f32;
            e2.sqrt()
        }
        let sparse = render(5.0);
        let dense = render(150.0);
        assert!(dense > 0.01, "粒子应出声,能量 {dense}");
        assert!(dense > sparse * 1.5, "密度 150 应明显比 5 响: dense={dense} sparse={sparse}");
    }
}
...
mod grain_env_tests {
    use super::*;
    #[test]
    fn grain_envelope_sweeps() {
        // 密度从 5 → 200 滑动:前期稀疏、后期密集 → 前 0.2s 能量远小于后 0.2s
        fn energy(p: &EngineParams, dur_s: f32) -> f32 {
            let mut e = SynthEngine::new(p.clone());
            e.note_on(60, 1.0, 0.0);
            ...
            let mut r = vec![0.0f32; n];
            e.render_block(&mut l, &mut r, n, 0.0);
            l.iter().map(|x| x * x).sum::<f32>() / n as f32
        }
        let mut p = EngineParams::default();
        p.wave_type = "grain".into();
        p.grain_density = 5.0;
        p.grain_density_end = 200.0;
        p.grain_env_ms = 1000.0;
        p.grain_size_ms = 60.0;
        p.grain_size_end = 60.0;
        let early = energy(&p, 0.2);    // 0-0.2s(密度仍低)
        let late = energy(&p, 0.5);     // 0-0.5s(含后期高密度段)
        // 直接比较:全段 0.5s 能量应明显高于 0.2s 段(后期变密)
        assert!(late > early * 1.5, "密度包络应显著改变能量 early={early} late={late}");
    }
}
